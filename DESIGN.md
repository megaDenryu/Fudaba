...
- 添付作成はPOST /items非対応。専用エンドポイント経由でのみ追加・削除する(`POST /api/fudaba/items/:id/attachments`、`GET /api/fudaba/attachments/:保存名`、`DELETE /api/fudaba/items/:id/attachments/:保存名`)
...
- UI: 詳細パネル・詳細シートにサムネイル一覧+ファイル選択(モバイルはカメラ/ライブラリが開く)+PC向けクリップボード貼り付け(本文欄にフォーカスしてCtrl+V)+原寸表示オーバーレイ(ページ遷移ゼロ原則を維持)+削除を提供
