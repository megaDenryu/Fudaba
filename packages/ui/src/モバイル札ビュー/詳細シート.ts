...
export class 詳細シート extends LV2HtmlComponentBase implements I配線可能<I詳細シート配線> {
  ...
  constructor() {
    ...
    this._本文 = textarea({ rows: 5, class: styles.シート本文入力 }).addTextAreaEventListener(
      "input",
      () => this._変更を検査する(),
    );
    this._種別選択 = select({
    ...
    this._担当者 = textInput({ placeholder: "未割当", class: styles.シート入力 })
  ...
  private _本体を構築する(
    ...
    return (
      div({ class: styles.シート本体 }).childs([
          ...
          種別選択,
          span({ text: "状態", class: styles.シートフィールドラベル }),
          状態,
