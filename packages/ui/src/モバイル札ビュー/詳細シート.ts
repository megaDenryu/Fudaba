...
import { チェックリスト編集 } from "../カンバン/チェックリスト編集";
...
export class 詳細シート extends LV2HtmlComponentBase implements I配線可能<I詳細シート配線> {
  ...
  private readonly _チェックリスト = new チェックリスト編集();
  ...
  constructor() {
    ...
    this._チェックリスト.変更時(() => this._変更を検査する());
    this._種別選択 = select({
