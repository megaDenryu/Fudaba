...
export class 作成シート extends LV2HtmlComponentBase implements I配線可能<I作成シート配線> {
  ...
  constructor() {
    ...
    this._タイトル = textInput({ placeholder: "タイトル", class: styles.シート入力 });
    this._本文 = textarea({
      placeholder: "本文（省略可）",
      rows: 3,
      class: styles.シート本文入力,
    });
    this._担当者 = textInput({ placeholder: "担当者（省略可）", class: styles.シート入力 })
    ...
    return (
      div({ class: styles.シート本体 }).childs([
          div({ class: styles.シートヘッダ }).childs([
              span({ text: "新しい札", class: styles.タイトル }),
              button({ text: "閉じる", class: styles.シート閉じるボタン }).onClick(() =>
                this._配線.先.on閉じる(),
              )]),
          span({ text: "種別", class: styles.シートフィールドラベル }),
          種別,
          span({ text: "タイトル", class: styles.シートフィールドラベル }),
          タイトル,
          span({ text: "本文", class: styles.シートフィールドラベル }),
          本文,
          span({ text: "担当者", class: styles.シートフィールドラベル }),
          担当者,
          担当者候補,
          span({ text: "ラベル", class: styles.シートフィールドラベル }),
          ラベル,
          ラベル候補,
          span({ text: "作成者", class: styles.シートフィールドラベル }),
          作成者,
          button({ text: "札を作成", class: styles.シート保存ボタン }).onClick(() =>
            this._作成を発火する(),
          )])
    );
  }
