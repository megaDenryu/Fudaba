...
import { 既定の稼働クライアントを作る, type 稼働クライアント } from "../通信/稼働クライアント";
...
import { 問いクライアント } from "../通信/問いクライアント";
import { 問い判定キュー } from "../問い判定キュー/問い判定キュー";
...
export class モバイル札ビュー extends LV2部品集約Base<モバイル札ビュー部品, モバイル札ビューサービス> {
  ...
  private readonly _問い判定キュー = new 問い判定キュー(new 問いクライアント());

  // キャラクライアント・稼働クライアントは省略時、自前でAgentRoomの/api/charas・
  // /api/presenceを叩く実装を構築する。既存の呼び出し元（Jimboシェル側コンポジション
  // ルート）が引数を増やさないままでも動く後方互換のため
  constructor(
    クライアント: 札クライアント,
    キャラクライアント: キャラクライアント = 既定のキャラクライアントを作る(),
    稼働クライアント: 稼働クライアント = 既定の稼働クライアントを作る(),
  ) {
    super();
    ...
    return (
      div({ class: styles.ルート }).childs([
          ...
          div({ class: styles.ヘッダ }).childs([
              span({ text: 文言.ヘッダタイトル, class: styles.タイトル }),
              button({ text: 文言.更新ボタン, class: styles.更新ボタン }).onClick(
                () => void サービス.更新する(),
              )]),
          部品.状態タブ.配線する({
          ...
          部品.詳細シート.配線する({
              ...
              on担当解除: (id) => void サービス.担当を解除する(id),
          }),
          部品.作成シート.配線する({
